            ...
            <div class="col-sm-12">
                ...
                <div class="card">
                    ...
                    <div class="card-block">
                        <form id="main" method="POST" action="{{ route('tasks.update', ['id' => $task->TaskId]) }}">
                            @csrf
                            <div class="form-group row">
                                <label class="col-sm-2 col-form-label">Tên công việc</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="TaskName" id="name"
                                        placeholder="Text Input Validation" value="{{$task->TaskName}}">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    ...
                                    <!-- Khu vực chỉnh sửa -->
                                    <div id="editor-container">{!! $task->Description !!}</div>
                                    <!-- Nút nhập file -->
                            ...
                            <div class="form-group row">
                                <label class="col-sm-2 col-form-label">Dự án</label>
                                <div class="col-sm-10">
                                    <select name="ProjectId" id="project-select" class="form-control">
                                        <option value="">-- Chọn dự án --</option>
                                        @foreach ($projects as $project)
                                            <option value="{{ $project->ProjectId }}"
                                                {{ $task->ProjectId == $project->ProjectId ? 'selected' : '' }}>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="datetime-local" class="form-control" id="repeat-password"
                                        name="StartDate" placeholder="Repeat Password" value="{{$task->StartDate}}">
                                    <span class="messages"></span>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="datetime-local" class="form-control" id="email" name="EndDate"
                                        placeholder="Enter valid e-mail address" value="{{$task->DueDate}}">
                                    <span class="messages"></span>
                                </div>
                            </div>
                            <div class="form-group row">
...
@section('scripts')
    ...
    <script>
        ...
        //         // Xóa hết thành viên cũ
        //         memberSelect.innerHTML = '<option value="">-- Chọn thành viên --</option>';

        //         if (projectId) {
        //             fetch(`/project/${projectId}/members`)
        //                 .then(response => response.json())
        //                 .then(data => {
        //                     // Giả sử bạn đã có biến $currentUsers từ PHP (chuyển qua JavaScript)
        ...
        //                         // Kiểm tra nếu UserId có trong $currentUsers, thêm selected
        //                         if (currentUsers.includes(member.UserId)) {
