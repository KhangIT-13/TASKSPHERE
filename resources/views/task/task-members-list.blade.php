            ...
            <div class="col-6 text-center">
                <div class="page-header-title">
                    <div class="d-inline">
                        ...
                        <span>Công việc : {{ $task->TaskName }}</span>
                    </div>
                </div>
            </div>
            <div class="col-3">
            ...
            <div class="col-sm-12 row">
                ...
                <div class="card col-6">
                    <div class="card-header">
                        <a href="{{ route('tasks.addMember',['TaskId' => $task->TaskId]) }}"><button class="btn btn-grd-success">Thêm thành viên mới</button></a>
                    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select name="role" class="form-control form-control-sm"
                                                    ...
                                                    <option value="Owner"
                                                        {{ $member->pivot->RoleInTask == 'Owner' ? 'selected' : '' }}>
                                                        Owner</option>
                                                    <option value="Reviewer"
                                                        {{ $member->pivot->RoleInTask == 'Reviewer' ? 'selected' : '' }}>
                                                        Reviewer</option>
                                                    <option value="Collaborator"
                                                        {{ $member->pivot->RoleInTask == 'Collaborator' ? 'selected' : '' }}>
                                                        Collaborator</option>
                                                </select>
                                            </td>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('select[name="role"]').on('change', function() {
                ...
                var taskId = $(this).data('task-id'); // Lấy ProjectId từ thuộc tính data nếu có
                var role = $(this).val(); // Lấy giá trị vai trò mới

                // Gửi AJAX để cập nhật vai trò
                $.ajax({
                    url: '{{ route('tasks.updateMemberRole') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        user_id: userId,
            ...
            $('.btn-delete-member').on('click', function() {
                ...
                customConfirm('Bạn có chắc chắn muốn xóa thành viên này không?', function(confirmed) {
                    if (confirmed) {
                        $.ajax({
                            url: '{{ route('tasks.deleteProjectMember') }}',
                            method: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                member_id: memberId,
                            ...
                            success: function(response) {
                                ...
                                $(`button[data-member-id="${memberId}"][data-task-id="${taskId}"]`)
                                    .closest('tr')
                                    .remove();
                            },
                            error: function() {
